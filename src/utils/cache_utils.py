...
def generate_achievable_hash(tier: int,
    ...
    tier, number of slots, max_copy, blacklist и props (name, low, high).
    """
    payload = {
        'tier': tier,
        'slots': slots,
...
def load_session_achievable(hash_key: str) -> Optional[Dict[str, float]]:
    """
...
def load_disk_achievable(tier: int, hash_key: str) -> Optional[Dict[str, float]]:
    """
    Ищет предрасчёт в JSON-файле для заданного тира на диске.
    """
    file_path = ACHIEVABLE_DIR / f'achievable_tier{tier}.json'
    if not file_path.exists():
        return None
    with file_path.open(encoding='utf-8') as f:
        all_data = json.load(f)
    entry = all_data.get(hash_key)
    return entry.get('maxima') if entry else None


def save_disk_achievable(tier: int, hash_key: str, data: Dict[str, float]) -> None:
    file_path = ACHIEVABLE_DIR / f'achievable_tier{tier}.json'
    ...
    if hash_key in all_data:
        return

    all_data[hash_key] = {'maxima': data}
...
def get_or_compute_achievable(settings: Any,
    ...
    result = load_disk_achievable(settings.tier, hash_key)
    if result is not None:
